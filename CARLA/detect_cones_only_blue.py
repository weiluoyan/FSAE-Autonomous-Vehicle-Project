...
def is_blue_object(cluster_points):
    """Assume blue objects have lower LiDAR intensity or use external camera info"""
    # If using LiDAR reflectivity (intensity)
    intensity_threshold = 0.2  # Adjust based on real data
    if np.mean(cluster_points[:, 2]) < intensity_threshold:  # Simulating blue detection
        return True
    return False

def process_lidar_data(point_cloud):
    """Process Hesai LiDAR data and detect blue cones."""
    ...
    # Filter valid clusters (only blue objects)
    valid_cones = []
    for label in set(labels):
        ...
        if np.sum(labels == label) >= 5 and is_blue_object(cluster_points):
            centroid = cluster_points.mean(axis=0)  # Compute cluster centroid
            valid_cones.append(centroid)

    rospy.loginfo(f"Detected {len(valid_cones)} blue cones.")
    track_cones(valid_cones)
    publish_cones(valid_cones)
...
def publish_cones(cone_positions):
    ...
    for idx, position in enumerate(cone_positions):
        ...
        marker.color.b = 1.0  # Only Blue color
...
def main():
    ...
    rospy.loginfo("Blue cone detection node started. Listening for LiDAR data...")
    rospy.spin()
