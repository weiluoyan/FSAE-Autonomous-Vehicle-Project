...
def process_camera_image(image_msg):
    """Process the camera image to detect orange objects."""
    global latest_image
    cv_image = bridge.imgmsg_to_cv2(image_msg, desired_encoding="bgr8")

    # Convert to HSV and apply color thresholding for orange
...
def is_orange_object(cluster_points, img_coords):
    """Check if a cluster corresponds to an orange object in the camera image."""
    if latest_image is None:
        return False  # No image available yet

    # Check if the corresponding pixels in the image are orange
    for (x, y) in img_coords:
        if 0 <= x < latest_image.shape[1] and 0 <= y < latest_image.shape[0]:  # Bounds check
            if latest_image[y, x] > 0:  # If pixel is orange in the mask
                return True
    return False
...
def process_lidar_data(point_cloud):
    """Process LiDAR data and detect orange cones."""
    ...
    for label in set(labels):
        ...
        # Only detect objects that are confirmed as orange by the camera
        if np.sum(labels == label) >= 5 and is_orange_object(cluster_points, img_coords):
            centroid = cluster_points.mean(axis=0)  # Compute cluster centroid
            detected_cones.append(centroid)

    rospy.loginfo(f"Detected {len(detected_cones)} orange cones.")
...
def track_cones(detected_cones):
    ...
    for cone in detected_cones:
        ...
        rospy.loginfo(f"Tracked orange cone position: {kf.x[:2]}")
...
def main():
    ...
    rospy.loginfo("Orange cone detection with LiDAR and camera started.")
    rospy.spin()
